...
For example, let's say that in our application we need to access a database that is running locally in our development environment but is running on a specific hostname in our production environment. We would like to be able to get  the correct hostname in our code, without too much effort, based on the current environment. Confippet can help us with this. Follow the setup instructions for this example depending on your app's framework:
...
First, lets update the default settings for our database connection by adding the following setting to `default.json`: 
...
Update `development.json` to include the following settings:
```json {
  "server": {
    "connections": {
      "compression": false
...
* The following is an example hapi.js app with [electrode-confippet] module fully integrated
...
* The following is an example express.js app with [electrode-confippet] module fully integrated
