...
### Running OTA
You can run OTA many ways, the easiest is to just 
```
 $ npm start
```
If you want to run in development mode, with the development config.
```
...
```
If your OTA server is not running over https you will need to add an exception to it in the Info.plist, use
Xcode to update.
```xml
<dict>
...
### Publishing (React)
To update the app over the air run.  See the Microsoft(tm) code-push docs for more information. On how to 
