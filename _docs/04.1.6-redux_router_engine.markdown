...
Now lets create the redux store using the configure function we just wrote. This pattern creates the store in a *server-side component* and initializes it with a promise library (the example below uses [bluebird](https://github.com/petkaantonov/bluebird/)):
...
To configure the Redux Rout0er Engine in an Express or Hapi application, first initialize the engine and then use it within a route handler to return the HTML. An example usage follows:
