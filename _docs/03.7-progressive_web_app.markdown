...
[logo 192x192](https://github.com/electrode-io/electrode-boilerplate-universal-react-node/blob/master/client/images/logo-192x192.png) and [logo 72x72](https://github.com/electrode-io/electrode-boilerplate-universal-react-node/blob/master/client/images/logo-72x72.png).
  We will be using these logos for `Add to Homescreen` banner and `Push Notifications`.

### Adding a Service Worker

Generating a service worker in an electrode app is as simple as adding a config file and registering the service worker on the browser. Navigate to `electrode-pwa/config` and create a new `sw-config.js`:
...
This will generate a `sw.js` in the `dist` folder when you build the app. Add it as a routes in `server/plugins/webapps/index.js`, inside the `registerRoutes` function:
...
And finally to register the service worker on the browser, create a new file `sw-register.js` inside the `client` directory:
